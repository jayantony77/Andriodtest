...
class ScoresScreen(MDScreen):

    def on_pre_enter(self):
        
        cluetext = str(self.ids.scorestb.cluetxt)
        teamclue = cluetext.partition("-")[0]
        inngclue = cluetext.partition("-")[2]

        self.ids.scorestb.title=teamclue + " Batting "
        ...
        if inngclue == "bat":
            ...
            self.ids.scorestb.title=teamclue + " Batting "
            if globalvar.current_team_batting == teamclue:
                ...
                for f_name in os.listdir(path):
                    if f_name.startswith('bat'):
                        ...
                        con += con
                        globalvar.current_team_batting=teamclue
